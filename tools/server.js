...
const PORT = cnf.network.develop.port;

// @TODO: check truffle contract as provider here
// @see https://github.com/trufflesuite/ganache-cli/blob/develop/README.md
const web3 = new Web3(new Web3.providers.HttpProvider('http://' + cnf.network.develop.host + ':' + PORT));
...
function startServer() {
    const config = {
        ...
        mnemonic: cnf.network.rinkeby.mnemonic,
        port: PORT,
        locked: false,
        // debug: true,
        gasPrice: cnf.network.develop.gasPrice,
        gasLimit: cnf.network.develop.gas,
    ...
    ganache.listen(PORT, async (err) => {
        if (err) {
            ...
            if (process.env.verbose) {
                ...
                log.info('Host\t: http://' + cnf.network.develop.host + ':' + PORT);
